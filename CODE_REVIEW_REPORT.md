...
| 文件 | 状态 |
|------|------|
| `static/js/app.js` 第302行 | ✅ 已修复：通过 `msg.source === 'online'` 判断 |

**完成操作**:
- ✅ 添加 `messages.source` 列（`local`/`online`）
- ✅ 修改 `save_message()` 支持 `source` 参数
- ✅ WebSocket 和 HTTP 接口的线上回复均标记 `source="online"`
- ✅ 前端恢复历史消息时正确读取 `source` 字段
