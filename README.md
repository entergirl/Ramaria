...
- 移动端适配
- 嵌入模型 CUDA 加速
- **前端管理面板**：集成安装/启动/配置功能到网页前端
  - 安装向导：环境检测 → 依赖安装 → 配置引导 → 数据库初始化
  - 服务控制：启动/停止/重启服务，实时状态监控
  - 配置管理：Env 在线编辑，必填项验证
  - 健康检查：六项自动验证，失败给出具体修复指引
  - 技术方案：后端管理 API (FastAPI) + 前端向导页 (原生 HTML/JS)
