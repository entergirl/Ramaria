...
与其他模块的关系：
  ...
  - 读 L1：database.get_l1_by_session() / database.get_latest_l1()
...
def _call_local_model(prompt):
    ...
    与 summarizer._call_local_model 逻辑一致，独立维护避免模块间耦合。
    ...
    payload = {
        ...
        "messages": [
            # /no_think 放在 system 层，尽量让模型跳过思考链直接输出 JSON
            # 注：Qwen3.5 不支持此指令，实际效果由 LM Studio 全局开关控制
    ...
    try:
        ...
        print("[conflict_checker] 错误：无法连接到 LM Studio，请确认模型已启动")
        return None
    except requests.exceptions.Timeout:
        print("[conflict_checker] 错误：请求超时（超过 60 秒）")
        return None
    except requests.exceptions.HTTPError as e:
        print(f"[conflict_checker] 错误：HTTP 请求失败 — {e}")
        return None
    except (KeyError, IndexError) as e:
        print(f"[conflict_checker] 错误：解析模型响应结构失败 — {e}")
        return None
...
def _strip_thinking(raw_text):
    ...
    逻辑与 summarizer._strip_thinking 完全一致，独立维护避免耦合。
    ...
    # 处理纯文本思考链：找到第一个 [ 或 { 之前的内容，视为思考链前缀截掉
    # 冲突检测输出是 JSON 数组，所以找 [ 而不是 {
...
def _parse_conflict_json(raw_text):
    ...
    本函数处理模型偶尔在 JSON 前后附加说明文字或思考链的情况。

    参数：
        raw_text — 模型返回的原始文本

    返回：
        list — 冲突对象列表，每个元素是包含四个字段的字典；
               无冲突时返回空列表 []；
               解析失败时返回 None（调用方需单独处理这种情况）

    返回示例（有冲突）：
        [
            {
                "field":        "personal_status",
                "old_content":  "最近在做毕业设计，压力较大",
                "new_content":  "烧酒说今天心情很好，感觉压力全消了",
                "conflict_desc":"之前记得你说毕业设计压力很大，但今天好像轻松了很多？是顺利了吗"
            }
        ]
    """
    # 第一步：剥离思考链
...
def handle_conflict_reply(conflict_id, action):
    ...
    调用示例：
        result_msg = handle_conflict_reply(conflict_id=3, action="resolve")
        # result_msg → "好的，我已经更新了你的近期状态记录。"
    """
    # 先读出这条冲突的完整信息
    pending = get_pending_conflicts()
...
if __name__ == "__main__":
    ...
    formatted = _format_profile_for_check(mock_profile)
    print(formatted)
    print()

    # ------------------------------------------------------------------
    # 测试二：冲突 JSON 解析（不依赖模型）
    # ------------------------------------------------------------------
    ...
    mock_conflict = '''[
      {
        "field": "personal_status",
        "old_content": "最近在做毕业设计，压力较大",
        "new_content": "烧酒说今天心情很好，压力全消了",
        "conflict_desc": "之前记得你说毕业设计压力很大，但今天好像轻松很多？是有什么好事发生吗"
      }
    ]'''
    result = _parse_conflict_json(mock_conflict)
    print(f"有冲突时解析：{len(result)} 条（应为 1）")
    print(f"  field = {result[0]['field']}（应为 personal_status）")

    # 空数组（无冲突）
    result2 = _parse_conflict_json("[]")
    print(f"无冲突时解析：{result2}（应为 []）")

    # 带思考链的输出
    mock_with_think = '<think>让我分析一下有没有矛盾。</think>\n[{"field":"personal_status","old_content":"压力大","new_content":"压力小","conflict_desc":"之前压力大，现在压力小？"}]'
    result3 = _parse_conflict_json(mock_with_think)
    print(f"带思考链的解析：{len(result3)} 条（应为 1，思考链应被剥离）")
    print()
    ...
    valid_item = {
        ...
        "old_content": "压力大",
        "new_content": "压力小",
        "conflict_desc": "之前说压力大，现在好像轻松了？"
    }
    print(f"合法条目：{_validate_conflict_item(valid_item)}（应为 True）")

    invalid_field = {**valid_item, "field": "mood"}   # 不在合法板块列表内
    print(f"非法 field：{_validate_conflict_item(invalid_field)}（应为 False）")

    missing_desc = {k: v for k, v in valid_item.items() if k != "conflict_desc"}
    print(f"缺少字段：{_validate_conflict_item(missing_desc)}（应为 False）")
    print()

    # ------------------------------------------------------------------
    # 测试四：get_conflict_question 和 handle_conflict_reply（需要数据库）
    # ------------------------------------------------------------------
    print("--- 测试四：冲突队列读写 ---")
    ...
    sid = new_session()
    save_message(sid, "user", "今天终于答辩完了，感觉整个人都轻松了")
    save_message(sid, "assistant", "太好了，辛苦你了！")
    close_session(sid)
    l1_id = save_l1_summary(
        session_id  = sid,
        summary     = "烧酒完成毕业答辩，心情轻松愉快。",
        keywords    = "毕业,答辩,轻松",
        time_period = "下午",
        atmosphere  = "轻松愉快",
    )
    print(f"已写入测试 L1 id={l1_id}，测试画像已初始化")

    # 手动写入一条模拟冲突（绕过模型调用）
    from database import save_conflict as db_save_conflict
    cid = db_save_conflict(
        source_l1_id  = l1_id,
        field         = "personal_status",
        old_content   = "最近在做毕业设计，压力较大",
        new_content   = "烧酒完成毕业答辩，心情轻松愉快。",
        conflict_desc = "之前记得你说毕业设计压力很大，但今天好像轻松很多了？是顺利答辩了吗",
    )
    print(f"已写入模拟冲突 id={cid}")

    # 读取待确认冲突
    result = get_conflict_question()
    if result:
        got_id, question = result
        print(f"\nget_conflict_question() 返回：")
        print(f"  conflict_id = {got_id}")
        print(f"  question    = {question}")
    print()

    # 处理"更新"回复
    msg = handle_conflict_reply(cid, action="resolve")
    print(f"handle_conflict_reply(resolve) → {msg}")

    # 确认队列已清空
    result2 = get_conflict_question()
    print(f"处理后 get_conflict_question() → {result2}（应为 None）")
    print()

    # ------------------------------------------------------------------
    # 测试五：端到端冲突检测（需要 LM Studio 运行中）
    # ------------------------------------------------------------------
    print("--- 测试五：端到端冲突检测（需要 LM Studio 运行中）---")

    # 写入一条新的 L1，内容和画像形成潜在矛盾
    update_profile_field("personal_status", "最近在做后端开发，状态很好，精力充沛")
    sid2 = new_session()
    save_message(sid2, "user", "今天太累了，什么都不想做，头疼得厉害")
    save_message(sid2, "assistant", "听起来今天很辛苦，要好好休息一下。")
    close_session(sid2)
    l1_id2 = save_l1_summary(
        session_id  = sid2,
        summary     = "烧酒今天疲惫不堪，头疼，情绪低落。",
        keywords    = "疲惫,头疼,情绪",
        time_period = "夜间",
        atmosphere  = "情绪低落",
    )
    print(f"已创建测试 L1 id={l1_id2}（与画像存在潜在矛盾）")
    ...
    if count is None:
        ...
        print("\n模型未检测到冲突（可能判断为语境不同，属正常情况）")
    else:
        print(f"\n端到端测试通过，检测到并写入了 {count} 条冲突")
        result3 = get_conflict_question()
        if result3:
            got_id3, question3 = result3
            print(f"询问文本：{question3}")
