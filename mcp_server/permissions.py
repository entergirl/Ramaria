"""
mcp_server/permissions.py — 工具权限边界配置
=====================================================================
...
def _load_env_overrides(permissions: dict[str, bool]) -> dict[str, bool]:
    ...
    """
    result = dict(permissions)  # 复制，不修改原始配置
...
# 运行时生效的权限表（已合并环境变量覆盖）
# server.py 和 tools/ 模块通过 import 这个变量来检查权限
...
def is_allowed(tool_name: str) -> bool:
    ...
    使用方法（在 write_tools.py 中）：
        if not is_allowed("update_profile"):
            raise PermissionError("update_profile 工具当前已禁用...")
    """
