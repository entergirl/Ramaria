"""
src/ramaria/core/session_manager.py — Session 生命周期管理
...
核心职责：
    ...
    3. 后台线程A（空闲检测）：每隔 IDLE_CHECK_INTERVAL_SECONDS 秒轮询：
           距上次消息超过 L1_IDLE_MINUTES 分钟
           → 关闭当前 session
           → 写入 L0 向量索引
           → 触发 L1 摘要生成
    4. 后台线程B（L2 定时检查）：每隔 L2_CHECK_INTERVAL_SECONDS 秒轮询：
           → 调用 merger.check_and_merge()（路径B：时间触发）
...
def trigger_l1_summary(session_id: int) -> None:
    """
...
class SessionManager:
    ...
    内部状态：
        ...
        _push_scheduler       — 主动推送调度器（由 start() 注入）

    典型使用流程：
        sm = SessionManager()
        sm.start()                    # 启动两个后台线程
        sid = sm.on_message()         # 每条用户消息到来时调用
        sm.stop()                     # 应用关闭时调用
    """
    ...
    def __init__(self):
        ...
        self._push_scheduler      = None  # 由 main.py lifespan 注入
    ...
    def stop(self) -> None:
        ...
        if self._idle_checker_thread and self._idle_checker_thread.is_alive():
            self._idle_checker_thread.join(
                timeout=IDLE_CHECK_INTERVAL_SECONDS + 1
            )
        if self._l2_checker_thread and self._l2_checker_thread.is_alive():
            self._l2_checker_thread.join(
                timeout=L2_CHECK_INTERVAL_SECONDS + 1
            )
    ...
    def force_close_current_session(self) -> None:
        """
    ...
    def _recover_active_sessions(self) -> None:
        ...
        if len(active) == 0:
            ...
            logger.warning(
                f"启动检查：发现 {len(active)} 个未关闭 session，全部关闭"
            )
            for s in active:
                sid = s["id"]
                close_session(sid)
    ...
    def _start_l2_checker(self) -> None:
        """创建并启动线程B（L2 定时检查线程）。"""
    ...
    def _idle_check_loop(self) -> None:
        ...
        """
        logger.debug(
            f"线程A（空闲检测）启动，轮询间隔 {IDLE_CHECK_INTERVAL_SECONDS} 秒"
        )
    ...
    def _check_idle_timeout(self) -> None:
        ...
        if idle_minutes >= L1_IDLE_MINUTES:
            logger.info(
                f"session {sid} 空闲超时（{idle_minutes:.1f} 分钟），触发关闭"
            )
            with self._lock:
    ...
    def _l2_check_loop(self) -> None:
        """
        线程B主循环：每隔 L2_CHECK_INTERVAL_SECONDS 秒执行一次 L2 触发检查。
        这是 L2 合并的路径B（时间触发）。
        """
        logger.debug(
            f"线程B（L2 定时检查）启动，轮询间隔 {L2_CHECK_INTERVAL_SECONDS} 秒"
        )
    ...
    def _trigger_l2_check(self) -> None:
        """
    ...
    def _close_and_summarize(self, session_id: int) -> None:
        """
        关闭 session，触发 L0 向量索引写入，再触发 L1 摘要生成。

        执行顺序：
            1. close_session()    — 打上 ended_at 时间戳
            2. index_l0_session() — 滑动窗口切片，写入 L0 向量索引
                                    必须在 L1 之前完成，
                                    L2 合并时检索链路需要 L0 已就位
            3. trigger_l1_summary() — 生成 L1 摘要，内部继续触发
                                      L1 向量索引写入和 L2 条数检查
        ...
        # L0 向量索引写入（失败只打警告，不阻断后续摘要生成）
        try:
            # 兼容导入：第三批迁移后 vector_store 已在新路径
            from ramaria.storage.vector_store import index_l0_session
            index_l0_session(session_id)
        except Exception as e:
            logger.warning(
                f"L0 向量索引写入失败，session_id={session_id} — {e}"
            )
